...
parser.add_argument('--model_A', type=str, default='./model_A.h5', required=False, help='Path to model A')
parser.add_argument('--model_B', type=str, default='./model_B.h5', required=False, help='Path to model B')
parser.add_argument('--pitch_style_ratio', type=float, default=0.5, required=False, help='style slide bar')
parser.add_argument('--tick_style_ratio', type=float, default=0.5, required=False, help='style slide bar')
...
pitch_style_B_rat = np.clip(args.pitch_style_ratio, 0, 1)
pitch_style_A_rat = 1. - pitch_style_B_rat
tick_style_B_rat = np.clip(args.tick_style_ratio, 0, 1)
tick_style_A_rat = 1. - tick_style_B_rat
...
def main():
    ...
    for i in range(noteNum):
        pred_note_A, pred_time_A = model_A.predict([notes, deltas], batch_size=1, verbose=0)
        pred_note_B, pred_time_B = model_B.predict([notes, deltas], batch_size=1, verbose=0)
        pred_note = pred_note_A * pitch_style_A_rat + pred_note_B * pitch_style_B_rat
        pred_time = pred_time_A * tick_style_A_rat + pred_time_B * tick_style_B_rat
